...
int main(int argc, char **argv)
{
    ...
    double xm, ym, line_a, line_b, line_c;
    if (simulation)
    {
        ...
        xm = x0 + half_wire_length * cos(yaw);
        ym = y0 + half_wire_length * sin(yaw);
        // line: ax + by + c = 0
        line_a = sin(alpha_wire);
        line_b = -cos(alpha_wire);
        line_c = ym * cos(alpha_wire) - xm * sin(alpha_wire);
